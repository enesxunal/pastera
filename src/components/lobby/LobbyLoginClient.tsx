...
export function LobbyLoginClient({ defaultBranchId }: { defaultBranchId?: string }) {
  const { t } = useI18n();
  ...
  async function submit(e: FormEvent) {
    ...
    const res = await fetch("/api/lobby/session", {
      method: "POST",
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-md flex-col justify-center px-6 py-16">
      <h1 className="font-display text-3xl font-bold text-white">{t("lobby.loginTitle")}</h1>
      <p className="mt-2 text-sm text-white/50">{t("lobby.loginHint")}</p>
